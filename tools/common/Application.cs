...
//			Console.WriteLine ("CopyFileCallback ({0}, {1}, 0x{2}, {3}, {4}, 0x{5})", what, stage, state.ToString ("x"), source, target, ctx.ToString ("x"));
		...
		public void RunRegistrar ()
		{
			...
			if (RootAssemblies.Count < 1)
				throw ErrorHelper.CreateError (129, "No assemblies found. You should provide at least one assembly.");
			...
			if (!foundProductAssembly)
				throw ErrorHelper.CreateError (130, "Product assembly '{0}' not found in assembly list: '{1}'", productAssembly, string.Join ("', '", RootAssemblies.ToArray ()));
